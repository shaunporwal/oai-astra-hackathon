...
## Visual integration shipped

The backup client now reuses the partner branch's Theme/AmbientBackground/glassCard implementation and a small subset of its button, chip and context components under lowercase `ios/backup/sources/theme.swift` and `components.swift`. Disabled primary buttons gain explicit dimming. The capture view adapts the partner's header, instruction hierarchy, rounded viewfinder, spacing and cards into a Capture / Review / Results flow. A persistent bottom action keeps capture/analysis reachable; camera exposure and server pairing move into a settings sheet. Endpoint details remain expandable.

This is a working visual adaptation in the existing native app, not a merge of the full partner application. Camera ownership, normalized ROI selection, shared API client and Python endpoints remain the existing implementations. No seeded participants, simulated clinical findings, assumed iris scale, native endpoint engine, or unsupported automatic-alignment status is imported. The theme is intentionally presented in light mode, matching the supplied visual design. Capture is still manual.
