...
## 2026-09-10 — Recurrent demo connection failure

Backend was listening and authenticated Mac-local checks succeeded, but the phone failed against the explicit Wi-Fi address with NSURLErrorDomain -1004. Added the underlying error domain/code and network-path description to local connection diagnostics. A signed build/install/launch passed; phone diagnostics showed a satisfied, viable Wi-Fi path with good link quality but failed server connection. Mac firewall was disabled; Tailscale LAN access was already enabled. Root cause remains unconfirmed. Requested a brief VPN pause/reconnect test; no VPN settings were changed without approval. No Astra calls were made during investigation. The mobile service is currently paired to the Mac Wi-Fi address rather than air.local.
