...
## Selected first workflow — clinician-supervised ocular redness follow-up

Project decision, 2026-09-10: prioritize guided capture of comparable conjunctival views for a clinician already following an ocular-surface condition. The initial clinical question is whether serial images are adequate for the clinician to grade visible redness. Quantitative vessel coverage is the first candidate measurement to evaluate against expert grades; it is not yet implemented or validated on our macro setup. Do not interpret a redness change as a treatment recommendation or a specific diagnosis.

This is selected for feasibility: visible tissue, no blood-test calibration and no timed light stimulus. Smartphone patient-capture evidence supports investigating the acquisition workflow, while slit-lamp hyperemia evidence supports the measurement hypothesis. Neither establishes this phone/attachment's monitoring performance. The first success criterion is improved independently graded capture adequacy versus unguided recording, followed by repeatability and grading agreement. A clinician must define the repeatable region and reference rubric before a clinical evaluation.

Use the rear camera with the macro attachment for the current prototype. “Front-facing view” means the lens is aimed approximately straight toward the eye; it does not mean the selfie camera. Verify the attachment is over the active rear lens. A centered overview supports alignment, but redness assessment additionally needs standardized exposed conjunctival views, not only a close pupil crop. Exact distance depends on the attachment and focus; no universal distance is assumed. Compare an overview and macro view before assuming the attachment improves grading.

Pupil geometry remains an engineering side task. Bilirubin, anemia, timed reflex and arcus/systemic-risk directions are deferred. Automatic diagnosis and urgent-care triage are outside this initial workflow.
