...
async function tick(e){if(!running||e!==epoch)return;try{const blob=await capture();const result=await (await api('/api/frame',{method:'POST',headers:{'content-type':'image/jpeg'},body:blob})).json();if(e!==epoch)return;draw(result);if(autoSelecting&&!saving&&!reviewing){const choice=selector.update(result,blob,performance.now());$('guidance').textContent=choice.message;if(choice.selected){autoSelecting=false;sync();await saveFrame(choice.selected,mode);}}$('tracking').textContent=result.pupil?'Pupil candidate':'No reliable candidate';$('diameter').textContent=result.pupil?`${result.pupil.diameter_px.toFixed(1)} px`:'—';$('sharpness').textContent=result.quality.laplacian_variance.toFixed(1);$('glare').textContent=`${(100*result.quality.bright_fraction).toFixed(2)}%`;$('latency').textContent=`${result.processing_ms.toFixed(0)} ms`;}catch(err){if(e===epoch){ctx.clearRect(0,0,overlay.width,overlay.height);$('tracking').textContent='Analysis unavailable';for(const id of ['diameter','sharpness','glare','latency'])$(id).textContent='—';message(err.message);}}if(running&&e===epoch)timer=setTimeout(()=>tick(e),200);}
function draw(r){overlay.width=r.image_size_wh[0];overlay.height=r.image_size_wh[1];ctx.clearRect(0,0,overlay.width,overlay.height);if(!r.pupil)return;const p=r.pupil;ctx.lineWidth=2;ctx.strokeStyle='#70ed99';ctx.beginPath();p.contour_xy.forEach(([x,y],i)=>i?ctx.lineTo(x,y):ctx.moveTo(x,y));ctx.closePath();ctx.stroke();ctx.strokeStyle='#67dfff';ctx.beginPath();ctx.ellipse(...p.center_xy,p.axes_wh[0]/2,p.axes_wh[1]/2,p.angle_degrees*Math.PI/180,0,Math.PI*2);ctx.stroke();}
